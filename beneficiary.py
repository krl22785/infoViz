...
def mergePatients(list):
	...
	for file in list:
                if index == 0:
                        ...
                        index += 1
			print "adding the %sth index" % index
	return df
...
def compoundPatients(df):
	...
	df_ben_grouped = df.groupby(['DESYNPUF_ID'])[col_diag]
	df_ben_summary = df_ben_grouped.agg('median')
	pd.DataFrame(df_ben_summary).to_csv("patient_summary.csv")
